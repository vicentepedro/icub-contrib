...
such as the iKinGazeCtrl under the main software branch and the 
GUI developed in MATLAB with the purpose to display the actual 
iCub gaze configuration (head posture and lines of sight). It 
requires the user just to connect some ports whose description 
...
public:
    GatewayThread(const int _period, const string &_portName, const string &_configFile,
                  const bool _headV2, const string &_visibility, const string &_pathToScript) :
...
public:
    ...
    virtual bool configure(ResourceFinder &rf)
    {
        ...
        string pathToScript=rf.findFile("script").c_str();
        if (!pathToScript.empty())
        {
            size_t npos=pathToScript.rfind("/"); 
            pathToScript=pathToScript.substr(0,npos);
        }

        thread=new GatewayThread(20,portName,configFile,headV2,visibility,pathToScript);
        if (!thread->start())
        {
            delete thread;
            return false;
...
int main(int argc, char *argv[])
{
    ...
    rf.setDefault("script","iKinGazeView.m");
