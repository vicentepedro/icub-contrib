...
public:
    GatewayThread(const string &_portName, const int _chn, const int _Fs,
                  ...
                  const string &_pathToScript) :
                  RateThread(1000/_Fs),    portName(_portName), chn(_chn),
                  Fs(_Fs),                 width(_width),       Nfft(_Nfft),
                  visibility(_visibility), pathToScript(_pathToScript)
    {
...
public:
    ...
    virtual bool configure(ResourceFinder &rf)
    {
        ...
        thread=new GatewayThread(portName,chn,Fs,width,Nfft,visibility,pathToScript);
        if (!thread->start())
        {
            delete thread;
            return false;
...
int main(int argc, char *argv[])
{
    ...
    rf.setDefault("script","stripchart.m");
